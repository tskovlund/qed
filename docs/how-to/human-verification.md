...
Human criteria default to `schedule = "manual"`, so they are excluded from `--auto`, `--extended`, and CI. They run during explicit invocation without flags, or with `--full`. See [How to add qed to CI](ci.md#ensuring-manual-criteria-get-validated) for recommendations on validating them regularly.
