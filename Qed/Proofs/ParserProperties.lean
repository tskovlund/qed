...
/-- `parseSchedule` accepts exactly the two valid schedule strings.
If it succeeds, the input was one of "always" or "manual". -/
theorem parseSchedule_complete (s : String) (cs : Schedule)
    (h : parseSchedule s = .ok cs) :
    s = "always" ∨ s = "manual" := by
...
/-- Any string that isn't "always" or "manual" produces an error. -/
theorem parseSchedule_rejects_invalid (s : String)
    (h1 : s ≠ "always") (h2 : s ≠ "manual") :
